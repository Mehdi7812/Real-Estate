<template>
    <header v-if="homePage_header" :style="`background: url(${customImage ? customImage : homePage_Pic});`" class="w-full h-5/6 bg-gray-400 relative !bg-cover !bg-top">
        <div class="backdrop-brightness-[.2] overflow-x-hidden">
            ...
            <div v-if="route.query.user" class="container m-auto mt-16 md:mt-20 lg:mt-0 p-6 lg:p-0 lg:py-5">
                ...
                <div v-if="userDatas" class="flex justify-center lg:justify-between gap-5 p-5 pb-20 dark:text-white lg:px-12 md:items-start lg:py-40">
                    <div class="flex flex-col lg:flex-row items-center gap-8">
                        <div class="flex items-center flex-col gap-8">
                            <img style="border-color: var(--primaryColor)" class="rounded-full border-2 lg:border-4 border-primaryOrange dark:border-bluePrimary w-20 h-20 lg:w-24 lg:h-24" :src="`${apiRootStore.api}/${userDatas.picture}`" :alt="userDatas.username" />
            ...
            <!-- Default Header -->
            <div v-else class="container m-auto mt-10 md:mt-14 lg:mt-0 p-6 lg:p-0 lg:py-5 dark:text-white">
                <div class="flex flex-col text-center items-center gap-5 p-5 pb-20 lg:px-12 lg:items-start lg:py-40">
                    <h2 class="mt-5 text-xs font-bold lg:text-[23px]">{{ homePage_header }}</h2>
                    <h1 v-if="!$route.params.length" class="text-2xl font-extrabold lg:text-6xl lg:text-right">{{ pageName }}</h1>
                    <p class="flex items-center gap-2">
