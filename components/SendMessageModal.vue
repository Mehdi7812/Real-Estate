...
const {idPostCase} = defineProps(['idPostCase'])

console.log(idPostCase);
...
const fetchSendMessage = () => {
    ...
    if(validateN() && validateP() && validateT()) {
        fetch(`https://api.hypomelk.ir/real/cases/${this.idPostCase}/conversation/`, {
            method: 'POST',
            headers: {
            ...
            if(res.status >= 200 && res.status < 400) {
                ...
                toast.success('پیام شما ارسال شد.', {position: toast.POSITION.BOTTOM_CENTER,autoClose: 40000,});
                emit('closeModalMessage')
            } else{
                toast.warning('لطفا دوباره تلاش کنید.', {position: toast.POSITION.BOTTOM_CENTER,autoClose: 40000,});
            }
        ...
        toast.warning('لطفا همه ی مقادیر را به درستی وارد کنید.', {position: toast.POSITION.BOTTOM_CENTER,autoClose: 40000,});
...
const emit = defineEmits(['closeModalMessage']);
...
<template>
    <!-- Modal Send Message -->
    <div id="modalMessage" class="transition-all duration-300 overflow-hidden">
