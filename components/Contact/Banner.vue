<template>
    <section class="px-4 py-24">
        <div v-if="dataRes" class="container m-auto">
            <h2 class="font-bold text-2xl mb-10 text-center md:text-[32px] md:mb-14">{{ dataRes.title }}</h2>
            <div :style="`background-image: url(${dataRes.banner})`" class="w-full h-36 bg-top bg-cover rounded-2xl md:h-80 md:rounded-[50px]"></div>
        </div>

        <div v-if="!dataRes" class="detailsLoading mt-10">
            <div id="card">
                <div class="card-image !h-44 md:!h-72">
